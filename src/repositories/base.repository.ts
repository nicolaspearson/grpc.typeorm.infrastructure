...
export default abstract class BaseRepository<T> {
	...
	public async executeRepositoryFunction(repositoryFunction: Promise<any>): Promise<any> {
		try {
			...
			if (error instanceof QueryFailedError) {
				throw GrpcBoom.internal(error.message);
			}
			throw GrpcBoom.invalidArgument(error);
		}
	}
	...
	public async findManyByFilter(options: FindManyOptions<T>): Promise<T[]> {
		...
		if (!records) {
			throw GrpcBoom.notFound(`${this.entityName}: The requested record was not found`);
		}
		return records;
	}
	...
	public async save(record: T, options?: SaveOptions): Promise<T> {
		...
		if (!result) {
			throw GrpcBoom.notFound(`${this.entityName}: The record was not saved: ${record}`);
		}
		if (result.id) {
	...
	public async saveAll(
		...
		if (!results) {
			throw GrpcBoom.notFound(`${this.entityName}: The records were not saved`);
		}
	...
	public async updateAll(
		...
		if (!results) {
			throw GrpcBoom.notFound(`${this.entityName}: The records were not updated`);
		}
